...
function initializeState(cb) {
  localStorage.set(config.extActivityStatusKey, false, (err, v) => {
...
function main() {
  ...
  chrome.action.onClicked.addListener(() => {
    localStorage.get(config.extActivityStatusKey, (err, v) => {
      const nextState = !v;

      console.log('Value currently is ' + v, 'next value ' + nextState);

      localStorage.set(config.extActivityStatusKey, nextState, (err, v) => {
